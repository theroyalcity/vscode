...
export class ExtensionHostConnection extends Connection {
	...
	public constructor(
		...
		protocol.sendControl(VSBuffer.fromString(JSON.stringify({ debugPort: this.params.port })));
	...
	private sendInitMessage(buffer: VSBuffer, inflateBytes: Uint8Array | undefined): void {
		...
		this.process.send({
			...
			inflateBytes: Buffer.from(inflateBytes || '').toString('base64'),
	...
	private async spawn(buffer: VSBuffer, inflateBytes: Uint8Array | undefined): Promise<cp.ChildProcess> {
		...
		proc.on('error', (error) => {
			console.log('EXTHOST DIED!', error);
			this.logger.error('Exited unexpectedly', field('error', error));
			this.dispose();
		});
		proc.on('exit', (code) => {
			console.log('EXTHOST EXITED!', code);
			this.logger.debug('Exited', field('code', code));
			this.dispose();
		});
		if (proc.stdout && proc.stderr) {
			proc.stdout.setEncoding('utf8').on('data', (d) => console.log(d));
			proc.stderr.setEncoding('utf8').on('data', (d) => console.error(d));
		}
		...
		proc.on('message', (event: ExtHostMessage) => {
			switch (event.type) {
				case '__$console':
					...
					if (fn) {
						fn.bind(this.logger)('EXTHOST console', field('arguments', event.arguments));
					} else {
